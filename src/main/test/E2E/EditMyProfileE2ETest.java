...
class EditMyProfileE2ETest extends ApplicationTest {
    ...
    @Test
    @Order(1)
    public void shouldGiveErrorAlertIfPasswordFieldIsEmpty(FxRobot robot) {
        loginUser(robot);
        WaitForAsyncUtils.sleep(5000, TimeUnit.MILLISECONDS);

        robot.clickOn("#profileTabId");
        ...
        robot.clickOn("#saveMyInfoButtonId");

        waitForCondition(Duration.ofSeconds(5), () -> {
            Optional<DialogPane> dialog = robot.lookup(".dialog-pane").tryQuery();
            if (dialog.isPresent() && dialog.get().getHeaderText().equals("Įvedimo klaida")) {
                return true;
            } else{
    ...
    @Test
    @Order(2)
    public void shouldGiveErrorAlertIfPhoneNumberFormatIsIncorrect(FxRobot robot) {
        loginUser(robot);
        WaitForAsyncUtils.sleep(5000, TimeUnit.MILLISECONDS);
        ...
        robot.clickOn(firstCell);
        robot.clickOn("#fieldPhone");
        TextField phoneNumField = robot.lookup("#fieldPhone").queryAs(TextField.class);

        robot.interact(() -> phoneNumField.clear());
        robot.interact(() -> phoneNumField.setText("86175789563"));
        robot.clickOn("#saveMyInfoButtonId");
